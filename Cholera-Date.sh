...
pages=$(curl 'http://eco.canadiana.ca/search?q=cholera&field=&df=1800&dt=1900&fmt=json' | ./jq '.pages')
...
for i in $(seq 10)
do
        curl 'http://eco.canadiana.ca/search/'${i}'?q=cholera&field=&df=1800&dt=1900&fmt=json' | ./jq '.docs[] | {published}' >> results.txt
done
